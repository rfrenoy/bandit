...
K = 3;
C = 5;                      % Number of contexts for exp4
T = 1000;                 % Horizon
verbose = false;           % Print ongoing results if set to true
bandit_types = char('dummy','softmax','softmax\_c','exp3','exp4');
...
for s=1:size(bandit_types,1)
    ...
    switch bandit_type
        ...
        case 'exp3'
            bandit = exp3(K,gamma);
        case 'exp4'
            experts = build_experts(C,K);
            bandit = exp4(K,gamma,experts);
        case 'softmax'
    ...
    for t=1:T
        ...
        if strcmp(bandit_type,'softmax\_c') == 1 || strcmp(bandit_type,'exp4') == 1
